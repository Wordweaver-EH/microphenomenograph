...
**IV bin schema.** Each IV's `levels` array must be non-overlapping by default — adjacent levels share at most one boundary value, treated as the start of the upper bin (right-open intervals). The validator rejects overlapping ranges unless the user passes `--allow-overlap` with a free-text rationale, persisted as `study.ivs[i].overlap_rationale`. The user's example bins `low 1-2 / med 3-4 / high 4-5` overlap at 4 and would be rejected with a named error pointing at the offending level; the user can either correct (e.g., `low 0-1 / med 2-3 / high 4-5` per the manual's example) or pass `--allow-overlap` with an explanation. Bins must also cover the full observed score range across the corpus or explicitly declare excluded scores via `study.ivs[i].excluded_scores: [...]`.
