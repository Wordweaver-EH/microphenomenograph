...
8. **Inter-rater reliability (IRR) is an early model-quality check with bootstrap CIs.** Per manual_kev.md, κ > 0.6 is the training-time threshold for analyst agreement. The pipeline runs **two automatic IRR checks per run** (after the first participant's diachronic, after the first participant's synchronic). Each check produces a structured report following Krippendorff's canonical procedure (union-of-categories coincidence matrix after alignment, per Krippendorff 2011) with **bootstrap 95% confidence intervals** (per Hayes & Krippendorff 2007, which the modal published practice skips but is methodologically defensible at small N).
   ...
   **JSONL record schema (with literature thresholds and pre/post-alignment α surfaced):**
   ```json
   {"stage":"diachronic","participant":"p1","suggestion":"s1","transcript_id":"p1s1","timestamp":"...",
...
### doc-as-done.AC23: Hypothesis evidence-class declaration is honest, not quota-filled
- **doc-as-done.AC23.1 Success:** Each candidate hypothesis in `hypothesis.candidate_drafting` artifacts carries a `supported_by` field naming the evidence classes that genuinely support it. Allowed values per class: an artifact-path citation, or `"not_applicable"` with a one-sentence rationale.
- **doc-as-done.AC23.2 Failure (review):** `hypothesis.weak_evidence_review` flags any hypothesis with causal language ("X causes Y", "X drives Y") whose `supported_by` map shows no concrete evidence citations (only `not_applicable` entries). Output as an `unsupported_causal_language` row in the review summary.
