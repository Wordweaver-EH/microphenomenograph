...
Three granularity and fidelity decisions distinguish this design from the previous stage-level model. First, the unit of "step" is the methodology's natural **substep**, not a stage — and the substep names follow manual_kev.md (Sheldrake & Dienes 2025) literally rather than the µ-PATH pipeline (Wordweaver-EH/upath, which encodes a different and less-simplified procedure). Diachronic decomposes into criteria-grouping → criteria-revision → IDU-naming-ordering with no sub-phase identification (the manual explicitly excludes it). Synchronic decomposes into theme-grouping-within-IDU → ISU-naming → ISU-2nd-level-grouping, iterated **per IDU**. Generic and global stages preserve the manual's IV-category × event × generic-IDU worksheet structure. Second, every LLM-invoking substep writes a **prompt-capture artifact** (`<scope>-<stage>.<substep>.prompt.json`) containing the exact prompt, response, model id, finish reason, and token counts. Audit events reference this file by path so any analytic decision can be replayed offline; fabrication becomes detectable by replay rather than merely rule-violating. Third, inter-rater reliability (IRR) is reframed as an **automatic model-quality gate**: after the first participant's diachronic completes, and again after the first synchronic completes, the orchestrator automatically runs an alternate-agent re-analysis and computes **partition-agreement metrics** between primary and alternate. Because two different LLMs typically invent their own category labels (primary's "Initial Sensation" vs alternate's "First Awareness"), the headline metric is the **Adjusted Rand Index (ARI)** — it compares the two partitions of the utterance set without using category labels at all. A partition-based Krippendorff's α (computed on pairwise "are these two utterances in the same group?" decisions) is reported alongside as a cross-check. Cohen's κ is only computed when category counts match AND name-alignment is unambiguous; otherwise it's null with a stated reason. Results append to `.mpi/irr_calibration.jsonl`; cross-participant stages emit `irr_warning` if low and proceed unless `--strict-irr`.
...
8. **Inter-rater reliability (IRR) is an early model-quality gate, computed twice automatically.** Per manual_kev.md, κ > 0.6 is the training-time threshold for analyst agreement; "an example should be used to compare the analyses … continue training until κ > .6". The pipeline operationalises this as **two automatic IRR checks per run**, each scheduled by the orchestrator without user prompting:

   - **After the first participant's `diachronic.idu_naming_ordering` close.** Orchestrator dispatches an alternate-agent re-analysis of the same transcript through all three diachronic substeps and computes IRR between primary and alternate IDU partitions.
   - **After the first participant's `synchronic.isu_second_level_grouping` close (over all that participant's IDUs).** Same pattern, for synchronic.

   **The IRR problem.** Primary and alternate produce their own category labels — primary's "Initial Sensation" and alternate's "First Awareness" might denote the same moment or not. Standard κ and standard nominal Krippendorff's α both require *matched* labels: pairs like (Initial Sensation, First Awareness) count as disagreement by name even when they describe the same partition. The pipeline therefore reports **three metrics, each suited to a different question**:

   - **Adjusted Rand Index (ARI)** — *primary metric*. Compares the two raters' partitions of the utterance set without using category labels at all. Asks: "do these two raters put the same utterances in the same groups, regardless of what they call the groups?" Chance-corrected (1.0 perfect, 0 random, negative worse than random). Robust to differing category counts. Computed in pure Python from a contingency table — no external dep.
   - **Krippendorff's α (nominal, label-free)** — computed not on raw labels but on the pairwise question "are utterances i and j in the same group?" for every pair, treating each pair-decision as a binary nominal coding. This is the partition-based α, equivalent in spirit to ARI but on a different formula. Reported alongside ARI as a cross-check.
   - **Cohen's κ** — *only computed when the two raters produce the same number of categories AND a unique optimal name-alignment exists* (single-pass Hungarian-algorithm assignment over IDU-name token overlap, only retained if the assignment cost is unambiguously low). Otherwise reported as `null` with reason `unmatched_categories` or `ambiguous_alignment`. This avoids the hack of forcing κ on misaligned categories; κ shows up only when it can be computed legitimately.

   All three metrics are appended to `.mpi/irr_calibration.jsonl`. The recorded line includes: `{stage, participant, primary_model, alternate_model, ari: float, alpha_partition: float, kappa: float|null, kappa_reason, n_units_primary, n_units_alternate, n_utterances, threshold: 0.6, outcome: "passed"|"low", timestamp}` — `outcome = "passed"` iff `ari >= 0.6` (the primary metric). κ and α are diagnostic information.

   Cross-participant stages emit an `irr_warning` audit event if the most-recent IRR record for the upstream stage is `low` or absent, **but do not block** — the warning is a model-quality signal ("consider an alternate primary model or revised prompts"), not a methodological gate on the run. Yolo proceeds. Users who want strict blocking pass `--strict-irr`.
...
### doc-as-done.AC13: IRR (ARI + α + optional κ) is an automatic model-quality gate
...
- **doc-as-done.AC13.2 Success:** The orchestrator automatically triggers `mpi-irr calibrate` after the **first** participant's `diachronic.idu_naming_ordering` closes, and again after the first participant's last `synchronic.isu_second_level_grouping` closes. No user invocation required.
- **doc-as-done.AC13.3 Success:** Each calibration appends a line to `.mpi/irr_calibration.jsonl` with `{stage, participant, primary_model, alternate_model, ari: float, alpha_partition: float, kappa: float|null, kappa_reason, n_units_primary, n_units_alternate, n_utterances, threshold: 0.6, outcome: "passed"|"low", timestamp}`. `outcome = "passed"` iff `ari >= 0.6`.
- **doc-as-done.AC13.4 Success:** ARI and partition-α are label-independent — they compute over the partition structure, not the category names — and are always reported even when primary and alternate disagree on IDU count.
- **doc-as-done.AC13.5 Success:** κ is only computed when `n_units_primary == n_units_alternate` AND the best Hungarian-algorithm name-alignment cost is ≤ 0.6 × runner-up cost; otherwise `kappa: null, kappa_reason ∈ {"unmatched_counts", "ambiguous_alignment"}`.
- **doc-as-done.AC13.6 Success:** Cross-participant skills emit `irr_warning` if the most-recent IRR record for the upstream stage is `low` or absent; proceed unless `--strict-irr`.
- **doc-as-done.AC13.7 Failure:** With `/mpi all --strict-irr` and missing or `low` IRR, cross-participant stages exit with a named ERROR before producing artifacts.
- **doc-as-done.AC13.8 Success:** `scripts/irr.py` unit tests cover: identical partitions → ari=alpha=1.0; random partitions → ari≈0; mismatched IDU counts → ari and alpha still computable, κ=null with reason; perfect partition agreement with different names → ari=1.0, κ may be null if names don't align.
...
- **IRR calibration (ARI primary; α and κ diagnostic)**: Automatic model-quality check triggered after the first participant's diachronic and after the first synchronic. Runs an alternate-agent re-analysis through the relevant substep DAG, then computes three metrics. Appends to `.mpi/irr_calibration.jsonl`. Outcome based on ARI ≥ 0.6. Warning by default; opt-in `--strict-irr` blocks.
- **Adjusted Rand Index (ARI)**: Compares two partitions of the same set of utterances without using category labels. Chance-corrected: 1.0 = identical partitions, 0 = random, negative = worse than random. The right metric when raters invent their own labels.
- **Partition-based Krippendorff's α**: α applied not to raw labels but to pairwise "are utterances i and j in the same group?" decisions. Equivalent in spirit to ARI; reported alongside as a cross-check.
- **Cohen's κ (only when alignable)**: κ requires matched categories between raters. The pipeline computes κ only when category counts match AND Hungarian-algorithm name-alignment is unambiguous (best cost ≤ 0.6 × runner-up cost). Otherwise reported as `null` with `kappa_reason ∈ {"unmatched_counts", "ambiguous_alignment"}`.
...
| `irr_calibration` | `independent_analyst` (LLM, alternate agent — re-runs all substeps of the gated stage), `agreement_computation` (orch — computes κ + α via `irr.py`) | auto-triggered after first participant's diachronic, and again after first synchronic; warning-by-default, opt-in `--strict-irr` |
...
- `global` — for the `weak_evidence_review` substep of hypothesis and for the `agreement_computation` half of `irr_calibration` (the `independent_analyst` half uses the same `pNsN` / `pNsN-iduN` scope as the primary substeps it shadows)
...
<!-- START_PHASE_13 -->
### Phase 13: IRR (κ + α) as automatic model-quality gate

**Goal:** Two automatic IRR checks per run — one after the first participant's diachronic completes, one after the first participant's synchronic completes — producing both Cohen's κ and Krippendorff's α. Warning-by-default; opt-in strict blocking via `--strict-irr`.

**Components:**
- `microphenomenograph/1.0.0/scripts/irr.py` — new module exposing `compute_irr(primary_assignments, alternate_assignments, level)` returning `{ari, alpha_partition, kappa, kappa_reason, n_units_primary, n_units_alternate, n_utterances}`. `level` is `"idu"` for diachronic (utterances → IDU group id) or `"isu"` for synchronic. Inputs are dicts `{utterance_id: group_label}` from each rater. Internally:
  - **ARI (primary metric):** Build contingency table from the two partitions, compute `ARI = (RI - E[RI]) / (max RI - E[RI])` using the standard combinatorial formula. Pure Python; no external dep.
  - **α (partition-based, nominal):** For every utterance pair, compute "are they grouped together?" as a binary 0/1. Apply nominal Krippendorff's α to the resulting pair-coding. Always reported.
  - **κ (label-aligned, optional):** Only computed when (a) `n_units_primary == n_units_alternate`, AND (b) a Hungarian-algorithm one-to-one assignment over IDU-name token-overlap similarity has an unambiguously low cost (best assignment cost ≤ 0.6 × runner-up cost). Otherwise κ = `null` with `kappa_reason ∈ {"unmatched_counts", "ambiguous_alignment"}`. This avoids forcing κ on mis-paired categories.
...
- Orchestrator hook in `/mpi all`: after the **first** participant's `diachronic.idu_naming_ordering` close, schedule `mpi-kappa calibrate --participant <that_pNsN> --stage diachronic` (renamed to `mpi-irr calibrate` for accuracy). After the first participant's last `synchronic.isu_second_level_grouping` close (across all that participant's IDUs), schedule `mpi-irr calibrate --participant <that_pNsN> --stage synchronic`. Both happen automatically — user does not invoke them.
- The calibrate operation runs the alternate-agent analysis through the same substep DAG (diachronic substeps 1-3 or all synchronic substeps for all IDUs), writes per-substep alternate artifacts to `analyses/independent/<pNsN>-<stage>.<substep>.{json,md,prompt.json}`, then computes IRR via `irr.py` and appends to `.mpi/irr_calibration.jsonl`.
- Cross-participant skills emit `irr_warning` audit event at stage start if the most-recent IRR record for the upstream stage is `low` or absent; proceed unless `--strict-irr`.
...
**Done when:** `mpi-irr calibrate` runs end-to-end on a fixture, produces alternate-agent artifacts for every diachronic (or synchronic) substep, and writes a JSONL line containing both `kappa` (or `null` with reason) and `alpha`. Auto-trigger fires once per stage per run after the first participant's relevant substep closes. Yolo with no IRR completes; yolo with `--strict-irr` and low/missing IRR errors before any cross-participant artifact. Verifies `doc-as-done.AC13.1`–`doc-as-done.AC13.7`.
<!-- END_PHASE_13 -->
