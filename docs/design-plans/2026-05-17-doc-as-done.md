...
- **doc-as-done.AC33.3 Success:** Default mode (empty target dir or pre-existing empty repo) writes `study.run_repo_mode = "dedicated"`. Helper sets `core.autocrlf false`, `core.eol lf`, recommends `commit.gpgsign false` (does not force) — local config only, never global.
...
What the helper does to the run repo: `git init` if needed, `git add` the initial transcripts + manifest + `.mpi/`, set `core.autocrlf false` and `core.eol lf` for cross-platform consistency, set a `commit.gpgsign false` recommendation (signing per substep is expensive; users who insist can override). It does NOT touch any global git config, only the run repo's local `.git/config`.
