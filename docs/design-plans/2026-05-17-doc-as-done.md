...
- **doc-as-done.AC16.6 Success (hypothesis evidence-source coverage, honest):** Each `hypothesis.evidence_extraction` artifact JSON contains an `evidence_sources` object with keys `generic_diachronic`, `generic_synchronic`, `global_synchronic`. Each key's value is **either** a non-empty array of citation paths, **or** the literal string `"not_applicable"` accompanied by a sibling `_rationale` key explaining why (e.g., `"global_synchronic": "not_applicable", "global_synchronic_rationale": "no shared generic IDU appears across multiple events in this corpus"`). Schema validation rejects empty arrays but accepts `not_applicable`-with-rationale. This supersedes the earlier draft's requirement of "non-empty arrays for all three" — that requirement was an evidence quota that incentivises junk citations, replaced by honest declaration (mirrors AC23.1).
