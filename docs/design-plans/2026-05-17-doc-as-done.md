...
8. **Inter-rater reliability (IRR) is an early model-quality gate, computed twice automatically.** Per manual_kev.md, κ > 0.6 is the training-time threshold for analyst agreement; "an example should be used to compare the analyses … continue training until κ > .6". The pipeline operationalises this as **two automatic IRR checks per run**, each scheduled by the orchestrator without user prompting:
   ...
   - **After the first participant's `diachronic.idu_naming_ordering` close.** Orchestrator dispatches an alternate-agent re-analysis of the same transcript through all three diachronic substeps and computes IRR between primary and alternate IDU sequences. This is the model-quality signal for the diachronic stage.
   - **After the first participant's `synchronic.isu_second_level_grouping` close (over all that participant's IDUs).** Same pattern, but on the synchronic outputs.

   IRR computation produces **both Cohen's κ and Krippendorff's α**:

   - **Cohen's κ** assumes a one-to-one mapping of categories between raters. When primary and alternate produce different numbers of IDUs (e.g., one says 4 moments, the other 5), κ requires a category-alignment step — implemented via Hungarian-algorithm assignment over IDU-name semantic similarity. If alignment fails or category counts diverge by more than 50%, κ is reported as `null` with reason `unalignable_categories`.
   - **Krippendorff's α** is computed over utterance-level codings (which IDU does utterance N belong to?) and is robust to differing category counts and missing data. α is always reported, even when κ is null.

   Both metrics are appended to `.mpi/irr_calibration.jsonl` (renamed from kappa_calibration.jsonl to reflect dual-metric scope). The recorded line includes: `{stage, participant, primary_model, alternate_model, kappa: float|null, kappa_reason, alpha: float, n_units_primary, n_units_alternate, threshold: 0.6, outcome: "passed"|"low", timestamp}` — `outcome = "passed"` iff `(kappa >= 0.6 OR kappa is null) AND alpha >= 0.6`.

   Cross-participant stages emit a `irr_warning` audit event if the most-recent IRR record for the upstream stage is `low` or absent, **but do not block** — the warning is a model-quality signal ("consider an alternate primary model or revised prompts"), not a methodological gate on the run. Yolo proceeds. Users who want strict blocking pass `--strict-irr`.
...
### doc-as-done.AC13: IRR (Cohen's κ + Krippendorff's α) is an automatic model-quality gate
...
- **doc-as-done.AC13.4 Success:** Krippendorff's α is always reported (robust to differing IDU/ISU counts between primary and alternate); Cohen's κ is `null` with `kappa_reason: "unalignable_categories"` when category counts diverge >50% or Hungarian-algorithm alignment fails.
- **doc-as-done.AC13.5 Success:** Cross-participant skills emit a `irr_warning` audit event at stage start if the most-recent IRR record for the upstream stage is `low` or absent. They proceed regardless.
- **doc-as-done.AC13.6 Failure:** When `/mpi all --strict-irr` is passed and the most-recent IRR is missing or `low`, cross-participant stages exit with a named ERROR before producing any artifacts.
- **doc-as-done.AC13.7 Success:** `scripts/irr.py` exposes a callable API: `compute_irr(primary_units, alternate_units, level: "idu"|"isu") -> dict`. Unit tests cover: identical inputs → κ=α=1.0; completely disjoint inputs → κ=α≈0; mismatched counts → κ=null with reason, α still reported.
...
- **IRR calibration (κ + α)**: Automatic model-quality check. Triggered by the orchestrator after the first participant's diachronic completes and again after the first synchronic. Runs an alternate-agent re-analysis through the same substep DAG, then computes Cohen's κ (with Hungarian-algorithm category alignment) and Krippendorff's α (robust to mismatched category counts). Appends to `.mpi/irr_calibration.jsonl`. Cross-participant stages emit `irr_warning` if outcome is low but proceed regardless; opt-in `--strict-irr` blocks.
- **Cohen's κ vs Krippendorff's α**: κ assumes raters use the same set of categories; when primary and alternate produce different numbers of IDUs/ISUs, κ requires a category-alignment step (and may report `null` if categories can't be aligned). α is computed at the utterance-level (which IDU does utterance N belong to?) and is robust to differing category counts, missing data, and is the more reliable signal when models disagree on segmentation granularity.
...
**Components:**
- `microphenomenograph/1.0.0/scripts/irr.py` — new module exposing `compute_irr(primary_units, alternate_units, level)` returning `{kappa, kappa_reason, alpha, n_units_primary, n_units_alternate}`. `level` is `"idu"` for diachronic (utterances → IDU assignments) or `"isu"` for synchronic (utterances → ISU labels). Internally:
  - Implements Cohen's κ on aligned categories, using Hungarian-algorithm assignment over IDU/ISU-name semantic similarity (uses simple token overlap; no embeddings dep). If category counts diverge by >50% or assignment cost exceeds threshold, κ = `null`, `kappa_reason = "unalignable_categories"`.
  - Implements Krippendorff's α (nominal, two-rater) directly per the standard formula. Robust to mismatched category counts. Always reported.
