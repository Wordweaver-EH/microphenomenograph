...
8. **Inter-rater reliability (IRR) is an early model-quality check with bootstrap CIs.** Per manual_kev.md, κ > 0.6 is the training-time threshold for analyst agreement. The pipeline runs **two automatic IRR checks per run** (after the selected calibration transcript's diachronic, after its synchronic). Each check produces a structured report following Krippendorff's canonical procedure (union-of-categories coincidence matrix after alignment, per Krippendorff 2011) with **bootstrap 95% confidence intervals** (per Hayes & Krippendorff 2007, which the modal published practice skips but is methodologically defensible at small N).
   ...
   **Bootstrap CIs.** For each metric, resample with replacement N=5000 times, recompute on each bootstrap sample, report the 2.5th and 97.5th percentiles as the 95% CI. Two resampling schemes by metric (AC32):
   - **α, κ, ARI** — naive utterance resampling (resampling unit = individual utterance). These operate on per-utterance label assignments where independence is fine.
   - **αU** — block bootstrap (block length ≈ √N, contiguous utterances). αU measures *boundary placement on a continuum*; independent utterance resampling destroys the contiguity αU is measuring, so naive bootstrap would over-estimate αU's CI tightness. The `bootstrap.method` field in each metric's record discloses which scheme was used.
   ~30–50 lines of pure-Python; no external dep.
...
10. **Every analytic claim cites transcript spans by byte offset, and the helper validates the spans exist.** The honest anti-fabrication mechanism: each IDU, ISU, ISU-2nd-level, GDU, GSS, global ISU, and hypothesis claim carries a `utterance_refs: [{transcript_id, utterance_number, byte_start, byte_end, raw_excerpt, display_char_start?, display_char_end?}, ...]` array. **Byte offsets are the binding contract** — they're unambiguous across Unicode, BOMs, normalisation, and avoid the Python-2-vs-3 / UTF-16 / grapheme-cluster pitfalls of character offsets. The helper resolves each ref through the `transcripts/offsets/<transcript_id>.json` registry built in `transcript_prep.register_offsets`, fetches the raw byte range from `transcripts/raw/<transcript_id>.txt`, computes its SHA, and rejects the close if any ref points outside the raw file, if `raw_excerpt` doesn't match the resolved bytes, or if the raw file's SHA doesn't match what's in the manifest. The optional `display_char_*` fields (computed by the helper at render time, never authored by the agent) carry human-readable character positions for UI display only. Empty `utterance_refs` is a schema validation failure for any generative substep — there is no "uncited claim" path.

**Utterance definition (deterministic and versioned).** An *utterance* is one numbered line in the **normalised** transcript produced by `transcript_prep.normalize`. The normalisation rule, fixed at `study.utterance_definition_version` in the manifest (currently `"v1"`): one speaker turn per line; if a speaker turn contains multiple sentences/clauses, the analyst-facing normalised form splits them into separate lines so each utterance is one clause-or-sentence per the manual_kev convention (manual: *"each line contains only a single utterance, such as a single clause from a sentence"*). The numbering starts at 1 after the header line and is dense (no gaps). Multi-line turns get sequential utterance numbers. The offset registry's keys are these utterance numbers; the values are byte ranges into the **raw** file (the raw may contain whitespace, BOMs, multi-clause turns that the normaliser split; the byte_start/byte_end span the raw bytes that contribute to that utterance, including leading whitespace and the trailing newline). Schemas reject `utterance_definition_version` mismatches between manifest and analyst output.
...
- **doc-as-done.AC7.2 Success:** Audit events follow the documented schema: `event_id` (UUID4), `@timestamp` (RFC3339 UTC), `trace_id`, `span_id`, `actor.{kind, name, model}`, `event.{kind, category, action, outcome, duration_ms}`, `mpi.{participant_id, suggestion_id, transcript_id, stage, substep, close_id, unit, manifest_status, artifact_paths, artifact_sha256, prompt_artifact_path, git_commit_sha, parent_head_sha}`, `reason`. The `event.action` enum includes the close phase markers: `close_attempted`, `artifacts_validated`, `audit_appended`, `manifest_replaced`, `git_commit_succeeded`, `git_commit_failed`, `manifest_rolled_back`. The `mpi.git_commit_sha` field appears only on `git_commit_succeeded` events (computable post-commit) — never on `manifest_replaced` (the self-reference impossibility). The `mpi.close_id` field is mandatory on every event emitted inside a close so phases can be joined. `event.duration_ms` is populated on `git_commit_succeeded` events (full close duration) and on per-substep `stage_completed`-class events where meaningful.
...
- **doc-as-done.AC13.5 Success:** Four metrics are reported, each with a 95% bootstrap CI from N=5000 resamples. Naive utterance resampling for α, κ, ARI; block bootstrap (block length ≈ √N) for αU per AC32. Same bootstrap sample indices reused across α/κ/ARI for cross-metric consistency; αU uses its own block-resample indices.
...
- **doc-as-done.AC20.5 Success (sequential yolo):** `/mpi all --yolo` dispatches one substep at a time. No two subagents run concurrently. Transcripts processed in deterministic order (manifest insertion order). Within a transcript, substeps follow the DAG; synchronic IDUs processed in IDU-number order. Audit log shows a strictly linear interleaving — no overlapping `stage_started`/`stage_completed` events for the same actor class.
- **doc-as-done.AC20.6 Failure (run lease):** Two concurrent `/mpi all` invocations on the same run directory: the second errors `run_lease_held` (immediately, no wait) and exits non-zero before any work begins. Lease auto-reclaimed only when the holder PID is verifiably dead on the same host.
- **doc-as-done.AC20.7 Failure (substep reservation):** Two `close` attempts for the same `(stage, substep, scope)` triple, even under a single run lease: the second errors `substep_reservation_held` before any LLM call begins. Reservation file `analyses/<scope>-<stage>.<substep>.reservation.json` is written before LLM work and deleted on `git_commit_succeeded` or `manifest_rolled_back`.
...
- **doc-as-done.AC29.4 Success (offset registry):** `transcript_prep.register_offsets` produces `transcripts/offsets/<transcript_id>.json` mapping normalized line numbers to raw `(line, byte_start, byte_end)` ranges; SHA256 recorded in the manifest. Every downstream `utterance_refs` resolves through it.
...
- **doc-as-done.AC33.4 Success (hooks disabled by default):** Helper sets local `core.hooksPath = .git/hooks-disabled` and creates that empty directory. If the user later sets it back, the next close at init records `study.git_hooks_enabled = true` and emits a `git_hooks_re_enabled` audit event noting the change.
- **doc-as-done.AC33.5 Success (signing off by default, with audit if enabled):** Helper sets `commit.gpgsign false` (recommendation, not forced). If the user enables signing, every subsequent `git_commit_succeeded` audit event records the signing key id under `mpi.git_signing_key_id`.
...
- **doc-as-done.AC33.7 Success (local-only by default):** The helper never runs `git push` or `git remote add`. If a remote exists at init (e.g., user cloned an empty repo from a remote), the manifest records `study.git_remote_configured = true` and the helper warns once: "MPI runs are designed to be local-only; pushing the run history to a CI-protected remote is unlikely to be what you want." The warning is non-blocking.

### doc-as-done.AC34: Study-config provenance is recorded; LLM proposal is opt-in
- **doc-as-done.AC34.1 Success:** At init, before any LLM call, the helper asks the user via AskUserQuestion: "How are IV scheme and DV foci provided?" with options `preregistered` / `user_specified` / `llm_proposed_user_confirmed`. The choice is persisted to `study.config_provenance` (immutable).
- **doc-as-done.AC34.2 Success (preregistered/user-specified paths):** When the user picks `preregistered` or `user_specified`, `init.propose_study_config` is **skipped** — the LLM never sees transcript content before IV/DV are finalised. The user supplies IV bins and DV foci directly via AskUserQuestion prompts; `init.confirm_study_config` echoes them back for confirmation.
- **doc-as-done.AC34.3 Success (LLM-proposed path):** Only when the user picks `llm_proposed_user_confirmed` does `init.propose_study_config` run — and even then, the user is shown the proposal with rationale and must accept/edit/replace before it's persisted. The manifest records both the LLM's original proposal AND the user's final answer so the difference is auditable.
- **doc-as-done.AC34.4 Documentation:** The `config_provenance` field is surfaced in every hypothesis-output disclaimer ("IV/DV scheme: <provenance>") so downstream readers can tell whether the study was confirmatory (preregistered), researcher-designed (user_specified), or exploratory (llm_proposed_user_confirmed). This prevents data-driven study design from being silently laundered into a confirmatory framing.
...
- **Bootstrap 95% CIs**: All four metrics carry confidence intervals from N=5000 bootstrap resamples per Hayes & Krippendorff (2007). α, κ, ARI use naive utterance resampling (shared indices for cross-metric consistency); αU uses block bootstrap (block length ≈ √N) because it measures contiguous-boundary placement, not point labels (AC32). Seed persisted in the record for reproducibility.
...
Every MPI pipeline step — whether executed by a subagent or by the orchestrator — closes by producing four artifacts under a transactional contract executed as a sequence of named phases. Each phase emits its own audit event so the close history is fully reconstructable even under partial failure:
...
**Cascade reset on revision.** Re-closing `diachronic.criteria_revision` for transcript `pNsN` triggers an automatic cascade in the manifest: `diachronic.idu_naming_ordering` for `pNsN` resets to `pending`; every `synchronic.*` substep scoped to a `pNsN-iduN` whose IDU was affected resets to `pending`; **all cross-participant substeps** (`generic_diachronic.*`, `generic_synchronic.*`, `global_synchronic.*`, `hypothesis.*`) reset to `pending` because their inputs are no longer current. The cascade emits one `cascade_reset` audit event per reset substep, with `mpi.cascade_source` referencing the triggering revision's `event_id`. The orchestrator MUST process these resets in a single manifest write before re-dispatching downstream work.

**Cascade reset moves artifacts aside, doesn't just flip status.** Marking the manifest `pending` without touching files leaves stale artifacts on disk where humans/scripts will keep opening them. The cascade therefore moves every affected `analyses/<scope>-<stage>.<substep>.{json,md,prompt.json}` triple to `analyses/_superseded/<cascade_close_id>/<scope>-<stage>.<substep>.*` (preserving relative path under `_superseded`). Each `_superseded/<close_id>/` directory carries a `tombstone.json` recording `{cascade_source: <revision_close_id>, reset_at: <ts>, reset_substeps: [...], reason: "<one-line>"}`. Moves are atomic per file (`os.rename`); the entire cascade is one git commit so the moves are reviewable in `git log`. `/mpi status` surfaces superseded counts (e.g., *"2 cascade events have moved 14 artifacts to _superseded/"*). This is structural, not cosmetic — it makes "fresh state after revision" actually fresh.
...
**Study configuration is user-driven, with explicit provenance tracking.** IV categories (e.g., response-score bins `low=0–1, moderate=2–3, high=4–5`) and DV foci (e.g., `cognition, emotion, sensations, imagination`) are study-specific. Per manual_kev.md, the IV is paired with the experimental design and the DV focus shapes interview probes and downstream analysis attention.

**Critical methodological design choice: avoid data-driven study design.** Letting an LLM propose IV/DV after sampling transcripts is convenient but leaks corpus content into the design — a confirmatory study should preregister IV/DV; an exploratory one can permit LLM proposal. `init` makes this distinction explicit, recorded as `study.config_provenance` in the manifest. Three substeps:

1. `init.scan_transcripts` (orchestrator) — copy transcripts into the run directory, parse headers (`Participant N, Suggestion N (Scored N/M)`), enumerate participants, suggestions, score values. **Never** passes transcript content to an LLM at this stage.
2. `init.propose_study_config` (orchestrator + maybe LLM, branching by provenance):
   - Helper first asks the user via AskUserQuestion: *"How are IV scheme and DV foci provided?"* with three options:
     - `preregistered` — IV/DV were chosen and documented before the study was run; user types them in. LLM never sees transcript content before IV/DV are finalised.
     - `user_specified` — researcher chooses now, without LLM assistance, but has not preregistered.
     - `llm_proposed_user_confirmed` — exploratory; let an LLM look at transcript samples and propose IV/DV; user confirms/edits.
   - Only on the third path does an LLM subagent run, given parsed headers + a small sample of utterances, to propose an IV scheme + DV foci with rationale. Manifest records both the LLM's original proposal AND the user's final answer.
3. `init.confirm_study_config` (orchestrator + user) — present the (proposed or user-typed) configuration via AskUserQuestion; user accepts, edits, or replaces. Final answer persisted into `study: {ivs, dvs, config_provenance, llm_proposal?}`. `config_provenance` is surfaced in every hypothesis output's disclaimer so downstream readers can tell whether the study was confirmatory, researcher-designed, or exploratory. DV foci are **attention hints**, not filters — they shape what `synchronic.isu_naming` and the `hypothesis.*` substeps emphasise, but never drop utterances or evidence from the analysis.
...
**Anti-fabrication, two layers.** Every generative substep gains an explicit anti-fabrication rule: missing or malformed upstream artifacts → return `ERROR`, never synthesize. The helper enforces this mechanically (schema validation, unknown-key rejection). Layer 1 is **call integrity** via `.prompt.json` replay — verifies the recorded prompt produces a comparable response. Layer 2 is **transcript-span grounding** (DoD #10 / AC28) — every analytic unit carries `utterance_refs` validated to resolve against the immutable raw transcript via the offset registry; helper rejects close on `missing_span_refs` / `span_out_of_range` / `span_excerpt_mismatch`. Layer 2 is what actually catches fabrication — a model that invents an unsupported claim either cites no spans, cites non-existent spans, or cites spans whose excerpts don't match. Both layers are mandatory in v1.
...
- The calibrate operation:
  ...
  4. Computes four metrics with bootstrap 95% CIs (N=5000 resamples, seed persisted for reproducibility): nominal α, Cohen's κ, ARI — naive utterance resampling; αU — block bootstrap with block length ≈ √N (AC32).
...
- **Run-level lease.** `/mpi all` acquires `.mpi/run.lease` at start — a file containing `{pid, hostname, run_id, started_at, command}`. The lease covers the entire orchestration, not just one close. Second `/mpi all` invocation while a lease is held errors `run_lease_held` immediately (no 30s wait — long blocks here suggest a real conflict). Same PID-verification/cross-host rules as the close lock. The lease is released cleanly on normal exit and by signal handlers on Ctrl-C; stale leases are reclaimed only when their PID is verifiably dead. Without a run lease, two concurrent `/mpi all` processes could read the same manifest, dispatch the same substep to two subagents, and race to close; the close lock prevents manifest corruption but not duplicate LLM work or duplicate artifact files.
- **Per-substep artifact-path reservation.** When a substep's pre-checks pass, `close` writes `analyses/<scope>-<stage>.<substep>.reservation.json` containing `{close_id, pid, started_at, intended_artifact_paths: [...]}` *before* the LLM call begins. A second `close` attempt for the same `(stage, substep, scope)` while a reservation exists errors `substep_reservation_held`. The reservation is deleted on `git_commit_succeeded` or `manifest_rolled_back`. This protects against double-dispatch within a run (e.g., a glitchy orchestrator) even when the run lease is held by a single process.
- **Close lock.** `.mpi/close.lock` serialises the manifest+audit+commit critical section (audit+manifest+commit MUST happen under one lock so the audit/manifest/git triple stays consistent). Held only for the duration of the phased close, not the full orchestration. PID-verified stale reclaim with `stale_lock_reclaimed` event; cross-host lock attempt errors `cross_host_lock_unresolvable`; manual recovery via `mpi_step.py unlock --reason ...`. A timestamp older than 60 minutes is recorded as a stderr warning by the *waiting* process (the audit-write itself is serialized by the same lock, so the waiter can't audit). The holder's `git_commit_succeeded` event records its full lock-held duration. Manual recovery path `mpi_step.py unlock --reason ...` overrides safety checks; emits `manual_unlock` with the reason.
...
**Operational requirement: dedicated run repo (enforced by init).** A 21-transcript run produces ~100–200 git commits (one per substep close). This volume conflicts with active development repos: pre-commit hooks fire per close, GPG signing prompts per close, CI triggers fire per close, branch protections block force-pushes, LFS quotas accumulate, dirty-worktree refusals fail. **`/mpi init` refuses by default to initialise a run inside an existing non-empty git worktree** — emits `run_inside_active_repo` with the recommendation: point `--run` at an empty directory; helper will `git init` if no repo is present. Override flag `--allow-active-repo-nested` exists for advanced users who have actually configured their hooks/CI/protections to coexist with MPI's commit volume; using the flag adds `study.run_repo_mode = "nested_in_active"` to the manifest so reviewers can later see the choice was conscious. The recommended pattern for operators who want MPI runs visible in their main repo: add the run directory as a **submodule** (one parent-repo commit per run completion), not as a nested directory.

What the helper does to the run repo: `git init` if needed, `git add` the initial transcripts + manifest + `.mpi/`, set `core.autocrlf false` and `core.eol lf` for cross-platform consistency. It does NOT touch any global git config, only the run repo's local `.git/config`.

**Git operational details (helper's defaults; each can be overridden in the local repo config).**

| Concern | Default | Override |
|---|---|---|
| **Pre-commit hooks** | Helper sets `core.hooksPath = .git/hooks-disabled` in the run repo's local config and creates that empty directory. Effectively disables all hooks for the run. Reason: a 5-second linter × 200 commits = ~17 minutes of pure hook overhead per study, and most hooks (linting, formatting, schema checks) are irrelevant to a research artefact repo. | `git config --local core.hooksPath .git/hooks` re-enables them. The override is auditable — `study.git_hooks_enabled` is recorded in the manifest when this is detected at init. |
| **GPG signing** | `commit.gpgsign false` set locally (recommendation level — does not force). Per-commit signing prompts would block every close and would create a usability cliff for unattended yolo runs. | User can `git config --local commit.gpgsign true` and configure a non-interactive signer (`gpg.program`, `gpg-agent` with cached passphrase, or SSH-key signing). The signing key id is recorded in audit events when enabled. |
| **CI triggers** | The run repo is **local-only by default** — no `git remote` is configured. No push, no CI. The helper never runs `git push`. | User can `git remote add origin <url>` and push manually outside the pipeline; CLAUDE.md notes that pushing a 200-commit MPI history to a CI-protected branch is the user's responsibility and almost certainly not what they want. The recommended visibility pattern is submodule (one parent commit per run completion). |
| **Branch protections** | N/A under default (local-only). If the user adds a remote and pushes, protections will reject the volume; that's the user's problem to solve. | Not the helper's concern. |
| **Author identity** | Inherits global `user.name` / `user.email`. If unset, init fails with `git_identity_unset` and prompts the user to set them locally for this run. The helper does NOT set a default author silently. | `git config --local user.name "..."` / `user.email "..."` per run. |
| **Commit signing for audit** | Not GPG signing; the audit chain itself (each commit's parent SHA recorded in the next manifest write's `parent_head_sha`) provides tamper-evidence. A rewritten history breaks the chain and is detected on next `close` via the HEAD-mismatch / parent_head_sha check. | — |
| **LFS** | Disabled. All artefacts are small JSON / markdown / JSONL; LFS would add a per-commit pre-push step with no benefit. | Users with large media (e.g., audio masters) should keep those outside the run repo. |
| **`.gitignore`** | Helper writes a starter `.gitignore` that excludes `__pycache__/`, `*.pyc`, OS noise (`.DS_Store`, `Thumbs.db`), and the close lock / reservation files (`.mpi/close.lock`, `.mpi/run.lease`, `analyses/*.reservation.json`). | User can extend; helper never edits user additions. |
