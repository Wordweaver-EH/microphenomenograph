...
8. **Cohen's κ is a one-shot setup sanity check, not a per-run gate.** Per manual_kev.md, κ > 0.6 is a training-time check on **one sample** between the primary analyst and an independent second analyst; the manual says "an example should be used to compare the analyses … continue training until κ > .6", not "block every production run". The pipeline reflects this: each `mpi-kappa calibrate` invocation produces an independent second analysis of one selected (participant, stage) pair using an alternate agent, computes κ, and **appends** a record to `.mpi/kappa_calibration.jsonll` (history, not single file — "continue training" implies repeatable attempts). Cross-participant stages emit a **warning** if the most-recent calibration is `low` or absent, but do not block — yolo runs to completion regardless. Users who want strict blocking can pass `--strict-kappa` to opt in.

9. **Hypothesis generation is three substeps, not one.** The manual frames hypothesis generation as patterns varying by IV level across generic diachronic, generic synchronic, and global synchronic outputs — synthesis, framing, and evidence audit are distinct cognitive loads that should not share a single LLM call. The three substeps: `hypothesis.evidence_extraction` (LLM, per DV focus — gathers pattern variations from upstream artifacts grouped by IV level, with full traceability back to source IDU/ISU artifact paths), `hypothesis.candidate_drafting` (LLM, per DV focus — drafts causal hypotheses framed against IV-level differences; cites evidence artifact paths), `hypothesis.weak_evidence_review` (LLM, global — flags hypotheses whose supporting evidence is thin given the participant count, mirroring the manual's "small number of participants … only provides weak evidence" guidance). Output: one `hypotheses/dv-<focus>.{candidates,review}.md` per DV focus plus a single global `hypotheses/review_summary.md`.
...
- **doc-as-done.AC20.4 Success:** A stale lock (PID not running OR timestamp older than 60 minutes) is auto-reclaimed with a `stale_lock_reclaimed` audit event; the new close proceeds. `mpi_step.py unlock --reason ...` manual path emits `manual_unlock` with the reason.
...
- **doc-as-done.AC21.3 Success:** `study.ivs[i].tie_resolution` is persisted in the manifest alongside `overlap_rationale`.
...
**IV bin schema.** Each IV's `levels` array must be non-overlapping by default — adjacent levels share at most one boundary value, treated as the start of the upper bin (right-open intervals; the boundary value belongs to the upper bin). The validator rejects overlapping ranges unless the user passes `--allow-overlap` with a free-text rationale AND a deterministic `tie_resolution` field per overlapping level: either `"upper"` (boundary value goes to higher level, the default), `"lower"`, or `"duplicate"` (the participant is assigned to BOTH levels — generic-diachronic worksheets show that participant in both IV-category rows; downstream cross-IV contrast counts the participant once per assignment with a `duplicate_assignment: true` audit annotation). The user's example bins `low 1-2 / med 3-4 / high 4-5` overlap at value 4: without `--allow-overlap`, rejected; with `--allow-overlap tie_resolution=upper`, value 4 → high; with `tie_resolution=duplicate`, the score-4 participant appears in both med and high. Bins must also cover the full observed score range across the corpus or explicitly declare excluded scores via `study.ivs[i].excluded_scores: [...]`.
...
- `skills/mpi-kappa/SKILL.md` rewritten as a calibration utility. One operation: `mpi-kappa calibrate --participant pNsN --stage diachronic|synchronic [--alternate-model <m>] [--alternate-prompt <variant>]`. Picks one (participant, stage) pair, runs the analysis a second time with the alternate agent, computes Cohen's κ via `scripts/kappa.py`, writes:
  ...
  - `.mpi/kappa_calibration.jsonll` — append-only history of calibration runs (one JSON line per run): `{participant, stage, primary_model, alternate_model, kappa, ci_lower, ci_upper, n_units, threshold: 0.6, outcome: "passed"|"low", timestamp, prompt_variant_notes}`. "Continue training until κ > .6" implies repeatable attempts; consumers read the last line as the current state.
  - One audit event `event.action: kappa_calibration` referencing both prompt-capture artifacts
- Cross-participant skills emit a `kappa_warning` audit event at stage start if `.mpi/kappa_calibration.jsonll` is missing for the upstream stage or its most-recent line's `outcome == "low"`. They proceed regardless unless `--strict-kappa` was passed at `/mpi all` invocation, in which case they error.
- `mpi-cross-analyst` agent prompt updated: read the last line of `.mpi/kappa_calibration.jsonll` if present and include its summary in its reasoning; do NOT block on it.
...
- **Concurrent close races.** The helper acquires `.mpi/close.lock` via `fcntl.flock` (POSIX) / `msvcrt.locking` (Windows) for the duration of audit+manifest+commit; a second `close` invocation while one is in flight blocks up to 30s, then errors `concurrent_close_blocked`. The lock file holds the holder's PID and ISO timestamp; on acquire-attempt the helper first reads the existing lock and, if the PID is not running (POSIX: `os.kill(pid, 0)` raises; Windows: `OpenProcess` returns null) or older than 60 minutes, emits a `stale_lock_reclaimed` audit event and overwrites the lock with the current PID. A manual unlock path `mpi_step.py unlock --reason ...` exists for cases where automatic detection fails; it emits a `manual_unlock` audit event with the reason. Prevents two parallel yolo runs from corrupting the manifest while still being resilient to crashed predecessors.
